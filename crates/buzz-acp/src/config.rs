...
/// Event kinds delivered by the default `subscribe=mentions` mode.
///
/// The list includes both canonical stream message kinds and forum posts and
/// comments because all three surfaces can carry a `#p` mention. Explicit
/// `--kinds` values still replace this list wholesale.
...
mod tests {
    ...
    fn test_mentions_mode_default_kinds() {
        ...
        for ch in &channels {
            ...
            assert!(kinds.contains(&buzz_core::kind::KIND_STREAM_MESSAGE_V2));
    ...
    #[test]
    fn test_dynamic_mentions_mode_uses_default_kinds() {
        let config = test_config(SubscribeMode::Mentions);
        let channel = Uuid::new_v4();

        let filter = resolve_dynamic_channel_filter(&config, channel, &[])
            .expect("mentions mode should subscribe to a discovered channel");

        assert_eq!(filter.kinds, Some(default_mention_kinds()));
